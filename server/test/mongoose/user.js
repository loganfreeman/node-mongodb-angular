...
describe( 'user schema', function() {
    ...
    it( 'should find user By Id', function(done) {
        ...
        var userNotExist = User.findById( '5522ccb251c4decbb4ec7bdde5' ).exec();
        Promise.all( [user, userNotExist] ).then( function(values) {
            console.log( 'values: \n' + values );
            //values[0].length.should.be.eq( 1 );
            done();
        } )
            .catch( CastError, function(err) {
                done();
            } )
            .catch( function(err) {
                done( err );
