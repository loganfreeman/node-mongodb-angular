            ...
            .then( function(user) {
                ...
                if (req.body.instances && req.body.instances.length) {
                    instancesToDelete = _.filter( user.instances, function(model) {
                        return !_.contains( req.body.instances, utils.safeToString( model ) );
                    } );
                    promises.push( user.update( {
                        $pullAll: {
                ...
                if (req.body.stacks && req.body.stacks.length) {
                    stacksToDelete = _.filter( user.stacks, function(model) {
                        return !_.contains( req.body.stacks, utils.safeToString( model ) );
                    } );
                    promises.push( user.update( {
                        $pullAll: {
...
var updateUserGroup = {
    spec: {
        // description: 'The method allows to retrieve items according to the given parameters',
        path: '/devops/user/{userId}/group',
        method: 'POST',
        notes: 'This method allows to update or modify the groups of a user',
        //summary: 'return items for the given criteria',
        //type: 'Category',
        ...
        parameters: [{
            ...
            paramTypes.body( 'body', '(object) Modified User Properties', 'UserGroupModification' )
        ],
    },
    action: function(req, res) {
        var promises = [];
...
var methods = [deleteEnvironment, deleteGroup, deleteUser, deleteStack, deleteInstance, deleteDeploy,
    ...
    updateUser, updateStack, updateInstance, updateDeploy, updateUserGroup
];
